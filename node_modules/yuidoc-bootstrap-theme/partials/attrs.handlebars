<div id="attr_{{name}}" class="attr item{{#if access}} {{access}}{{/if}}{{#if deprecated}} deprecated{{/if}}{{#if extended_from}} inherited{{/if}}">
    ...
    {{#if optional}}
        <span class="flag optional">опциональный</span>
    {{/if}}

    {{#if required}}
        <span class="flag required">обязательный</span>
    {{/if}}

    {{#if readonly}}
        <span class="flag readonly">только для чтения</span>
    {{/if}}
    ...
    {{#if emit}}
        <div class="emits box">
            <h4>Генерирует событие <code>{{name}}</code></h4>

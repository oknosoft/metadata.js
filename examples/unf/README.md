...
Рассмотрим сначала демо-пример в облаке. Затем - развернём тот же пример на локальном компьютере.

## Шаг №1 - структура и файлы
Цель:
- Понять, какие файлы необходимы и достаточны для старта приложения на metadata.js
- Убедиться, что интерфейс из автосгенерированных форм:
    + Работоспособен, позволяет читать и редактировать справочники и документы
    + Не требует написания клиентского кода

Читаем javascript [на странице примера](http://jsfiddle.net/oknosoft/yza3msa9/). Существенные для понимания моменты:

#### Подключение _metadata.js_
```html
<script src="//oknosoft.github.io/metadata.js/lib_ext/dhtmlx.min.js"></script>
...
Подход, при котором описание метаданных разделено на два файла, позволяет упростить разработку и сопровождение, т.к. файл `meta.json` формируется автоматически обработкой [ИнтеграцияОписаниеМетаданныхУНФ.epf](data/ИнтеграцияОписаниеМетаданныхУНФ.epf), а файл `meta_patch.json` предназначен для редактирования разработчиком. Второй файл потребовался, чтобы дополнить метаданные свойствами, которые нельзя задать или переопределить в конфигураторе 1С. Например, ширины и видимость колонок табличной части объекта или признак кеширования.
...
```javascript
prm.data_url = "//www.oknosoft.ru/assets/examples/unf/data/";
```
...
Когда пользователь открывает страницу приложения на базе `metadata.js`, в памяти компьютера и на экране разворачивается примерно такая (укрупненно) цепочка событий:
- Браузер читает html заглавной страницы и загружает в память объявленные на этой странице файлы стилей css и скрипты js. Загруженные скрипты сразу выполняются. Создаются структуры, в которые после авторизации будут загружены данные приложения, назначаются обработчики событий браузерного объекта window. В процессе выполнения, могут быть загружены дополнительные скрипты и стили
...
- Процедура `$p.iface.oninit`
    ...
    + Создаёт стандартный диалог авторизации. Если приложение рассчитано на анонимных пользователей, либо в случае, если авторизация в сервисе выполняется иными средствами, авторизацию средствами `metadata.js` можно пропустить
- После ввода имени пользователя и пароля и нажатия на кнопку `Войти`, выполняются стандартные шаги инициализации объектов данных. Подробнее см. процедуру [log_in модуля events](http://www.oknosoft.ru/upzp/apidocs/classes/AppEvents.html#method_log_in)
    ...
    + Диалог авторизации удаляется из памяти, а управление передаётся клиентскому коду, определенному в `$p.iface.oninit`. В нашем примере, этот код устанавливает в адресной строке браузера `hash#obj=doc.ЗаказПокупателя&view=oper`
- Браузер, обнаружив изменения в адресной строке, сообщает о них в процедуру `$p.iface.hash_route`, которая выполняет маршрутизацию: показывает форму объекта или списка или выполняет иные действия в зависимости от хеша url. В нашем случае, открывается форма списка документов _Заказ покупателя_
...
- Дополнить описание метаданных объекта. Например, для формы списка справочника `Номенклатура`, в файле [meta_patch](data/meta_patch.json), указано создать пять колонок (Код, Артикул, Наименование, Ед и Номенклатурная группа). Т.к. поля `Единица измерения` и `Номенклатурная группа` ссылочного типа и элемент номенклатуры ничего не знает про наименования этих элементов, в разделе `fields` того же описания, строки `"cat_КлассификаторЕдиницИзмерения".name as "ЕдиницаИзмерения"` и `"cat_НоменклатурныеГруппы".name as "НоменклатурнаяГруппа"` приводят к тому, что в запрос к справочнику `cat_Номенклатура` будут добавлены таблицы `cat_КлассификаторЕдиницИзмерения` и `cat_НоменклатурныеГруппы`, и в качестве текста колонок `ЕдиницаИзмерения` и `НоменклатурнаяГруппа`, будут использованы значения полей `name` связанных таблиц
...
Рассмотрим модификацию с помощью [meta_patch](data/meta_patch.json) еще раз на примере формы объекта документа `ЗаказПокупателя`
- В разделе form->obj->head, (шапка документа) определены три группы полей
    + Группа без заголовка с полями `Номер`, `Дата`, `ВидОперации`, `Организация`, `Контрагент` и `Договор`
    + Группа `Планирование` с полями `СостояниеЗаказа`, `Старт`, `Финиш`
    + Группа `Дополнительно` с полями `Проект`, `Касса`, `Событие`, `НалогообложениеНДС`, `СуммаВключаетНДС`, `Ответственный`, `Комментарий` и `СуммаДокумента`
...
- В разделе form->obj->tabular_sections, указано отображать только табличную часть `Запасы`, а табличные части `МатериалыЗаказчика`, `ПлатежныйКалендарь`, `СкидкиНаценки` и т.д. - скрыть
- Для табличной части `Запасы` явно указан состав колонок, их заголовки, типы элементов управления и ширины по умолчанию
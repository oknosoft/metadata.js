/*!
 metadata-pouchdb v2.0.1-beta.18, built:2017-07-15
...
const plugin_http = require('pouchdb-adapter-http');
const plugin_replication = require('pouchdb-replication');
const plugin_mapreduce = require('pouchdb-mapreduce');
const plugin_find = require('pouchdb-find');
const plugin_memory = require('pouchdb-adapter-memory');
const plugin_websql = require('pouchdb-adapter-websql');
const plugin_idb = require('pouchdb-adapter-idb');
const plugin_authentication = require('pouchdb-authentication');
PouchDB
	.plugin(plugin_http)
	.plugin(plugin_replication)
	.plugin(plugin_mapreduce)
	.plugin(plugin_find);
if(typeof process !== 'undefined' && process.versions && process.versions.node){
	PouchDB.plugin(plugin_memory);
}
else{
	PouchDB.plugin(plugin_authentication);
	const ua = (navigator && navigator.userAgent) ? navigator.userAgent.toLowerCase() : '';
	if(ua.match('safari') && !ua.match('chrome')){
		PouchDB.plugin(plugin_websql);
	}else{
		PouchDB.plugin(plugin_idb);
	}
}
class AdapterPouch extends metadataAbstractAdapter.AbstracrAdapter{
	constructor($p){
		super($p);
		var t = this,
			_paths = {},
...
const plugin = {
	constructor(){
		const {constructor, adapters, wsql} = this;
		const {DataObj, DocObj, TaskObj, BusinessProcessObj} = constructor.classes;
		Object.defineProperty(adapters, 'pouch', {value: new AdapterPouch(this)});
		Object.defineProperty(constructor.classes, 'PouchDB', {value: PouchDB});
